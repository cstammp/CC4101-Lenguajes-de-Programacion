...
<type>   ::= NumT | BoolT | {PairT <type> <type>}
...
;; Parsea un tipo en una estructura Type.
(define (parse-type src)
  (match src
    ['Num (numT)]
    ['Bool (boolT)]
    [(list 'Pair t1 t2) (pairT (parse-type t1) (parse-type t2))]))

; parse-arg :: s-expr -> Arg
;; Parsea un argumento (id : Tipo) en una estructura Arg.
(define (parse-arg src)
  (match src
    [(list id ': type) (arg id (parse-type type))]))
  ...
  (match e
    ...
    [(app f exprs)
      (match (lookup-fundef f funs)
        [(fundef _ args _ body)
              (let* ([arg-vals (map (lambda (e) (interp e env funs)) exprs)] [arg-ids (map arg-id args)])
                (if (not (= (length arg-ids) (length arg-vals)))
                  (error (format "Arity mismatch: function bar expected ~a arguments, received ~a" (length arg-ids) (length arg-vals)))
                  (let* ([env-extended (foldr (lambda (bind env)(extend-env (car bind) (cdr bind) env))
                                        empty-env ;; scope léxico
                                        (map cons arg-ids arg-vals))])
        (interp body env-extended funs))))])]
    [_ (error "not yet implemented")]
    ))

;; run :: s-expr -> Val | error
;; Recibe un programa en notación s-expr, lo parsea, checkea los tipos, lo interpreta y retorna
;; su resultado como un Val. Lanza errores si el programa no es válido.
(define (run src)
  (let* ([p (parse-prog src)])
    (typecheck p)
    (let* ([expr (prog-main p)]
...
;; list-first-mismatch :: List[Any] List[Any] -> (cons Any Any) | #f
;; Devuelve el primer par de tipos que no coinciden, o #f si todos coinciden
(define (list-first-mismatch expected actual)
  (match (list expected actual)
    [(list '() '()) #f]
    [(list (cons e erest) (cons a arest)) (if (equal? e a) (list-first-mismatch erest arest) (cons e a))]
    ))

;; typecheck-binding :: Binding Env List[Fundef] → Type | error
;; Checkea y retorna el tipo de un Bind o error
(define (typecheck-binding bind env funs)
  (match bind
    [(binding id type expr) (let ([exprtype (typecheck-expr expr env funs)]) 
                            (if type (if (equal? type exprtype) type 
                                                                (error (format "Static type error: expected ~a found ~a" (pp-type type) (pp-type exprtype))))
                                      exprtype))
    ]))


;; typecheck-expr :: Exp Env List[Fundef] → Type | error
;; Checkea y retorna el tipo de una expresión o error
(define (typecheck-expr e env funs)
  (match e
    [(num n) (numT)]
    [(bool b) (boolT)]
    [(id x) (env-lookup x env)]
    [(pair l r) (pairT (typecheck-expr l env funs) (typecheck-expr r env funs))]
    [(add-one e)
        (match (list(typecheck-expr e env funs))
          [(list (numT)) (numT)]
          [e (error (format "Static type error: expected Num found ~a" (pp-type (checktype numT? e))))])]
    [(add l r)
        (match (list (typecheck-expr l env funs) (typecheck-expr r env funs))
          [(list (numT) (numT)) (numT)]
          [e (error (format "Static type error: operator + expected Num found ~a" (pp-type (checktype numT? e))))])]
    [(sub l r)
        (match (list (typecheck-expr l env funs) (typecheck-expr r env funs))
          [(list (numT) (numT)) (numT)]
          [e (error (format "Static type error: operator - expected Num found ~a" (pp-type (checktype numT? e))))])]
    ...
    [(eq l r)
        (match (list (typecheck-expr l env funs) (typecheck-expr r env funs))
          ...
          [e (error (format "Static type error: operator = expected Num found ~a" (pp-type (checktype numT? e))))])]
    [(not-new b)
        (match (list(typecheck-expr b env funs))
          [(list (boolT)) (boolT)]
          [e (error (format "Static type error: operator ! expected Bool found ~a" (pp-type (checktype boolT? e))))])]
    [(and-new l r)
        (match (list (typecheck-expr l env funs) (typecheck-expr r env funs))
          [(list (boolT) (boolT)) (boolT)]
          [e (error (format "Static type error: operator && expected Bool found ~a" (pp-type (checktype boolT? e))))])]
    [(or-new l r)
        (match (list (typecheck-expr l env funs) (typecheck-expr r env funs))
          [(list (boolT) (boolT)) (boolT)]
          [e (error (format "Static type error: operator || expected Bool found ~a" (pp-type (checktype boolT? e))))])]
    [(fst p)
        (match (list(typecheck-expr p env funs))
          [(list (pairT lt rt)) lt]
          [e (error (format "Static type error: expected Pair found ~a" (pp-type (checktype pairT? e))))])]
    [(snd p)
        (match (list(typecheck-expr p env funs))
          [(list (pairT lt rt)) rt]
          [e (error (format "Static type error: expected Pair found ~a" (pp-type (checktype pairT? e))))])]
    [(if-new c t f)
        (match (list(typecheck-expr c env funs))
          [(list (boolT)) (let ([tt (typecheck-expr t env funs)] [ft (typecheck-expr f env funs)])
                          (if (equal? tt ft)
                              tt
                              (error "las ramas del if deben tener el mismo tipo")))]
          [e (error (format "Static type error: expected Bool found ~a" (pp-type (checktype boolT? e))))])]
    [(with bindings body) (typecheck-expr body (foldr (lambda (bind env) (extend-env (binding-id bind) (typecheck-binding bind env funs) env)) env bindings) funs)]
    [(app f exprs)
      (match (lookup-fundef f funs)
        [(fundef _ args type body)
              (let* ([expected-types (map arg-type args)] [arg-types (map (lambda (e) (typecheck-expr e env funs)) exprs)] [arg-ids (map arg-id args)])
                (if (not (= (length expected-types) (length arg-types)))
                  (error (format "Arity mismatch: function bar expected ~a arguments, received ~a" (length expected-types) (length arg-types)))
                  (let ([mismatch (list-first-mismatch expected-types arg-types)])
                    (if mismatch
                      (error (format "Static type error: expected ~a found ~a" (pp-type (car mismatch)) (pp-type (cdr mismatch))))
                      (let* ([env-extended (foldr (lambda (bind env)(extend-env (car bind) (cdr bind) env))
                                            empty-env ;; scope léxico
                                            (map cons arg-ids arg-types))])
        (typecheck-expr body env-extended funs))))))])]
    [_ (error "not yet implemented")]
    ))

;; typecheck-fundef :: Fundef List[Fundef] → Type | error
;; Checkea y retorna el tipo de una función o error
(define (typecheck-fundef f funs)
  (match f
    [(fundef name args type body)
      (let* (
            [arg-ids (map arg-id args)]
            [arg-types (map arg-type args)]
            [env-extended (foldr (lambda (bind env) (extend-env (car bind) (cdr bind) env))
                          empty-env
                          (map cons arg-ids arg-types))]
            [body-type (typecheck-expr body env-extended funs)])
        (if (equal? type body-type)
          type
          (error (format "Static type error in function ~a: expected ~a found ~a" name (pp-type type) (pp-type body-type)))))]
    [_ (error "not yet implemented")]))

;; typecheck :: Prog → Type | error
;; Checkea y retorna el tipo de un programa o error
(define (typecheck prg)
  (match prg
    [(prog fundefs main) (for-each (lambda (fundef) (typecheck-fundef fundef fundefs)) fundefs) 
                         (typecheck-expr main empty-env fundefs)]
    [_ (error "not yet implemented")]))